...
class InstaParse:
    ...
    def __init__(self):  # , cursor, name_of_database
        ...
        self.username = config.get("poh","poh_name")
        self.password = config.get("poh","poh_key")
        confirm = input('Would you like to login with {}? (y/n)'.format(self.username)).strip().lower()
        if confirm == 'y':
            self.url = "http://instagram.com/accounts/login"
            self.path = 'C:\\Users\\cass\\AppData\\Local\\Programs\\Python\\Python36\\Scripts\\chromedriver.exe'  # location of the chromedriver for selenium
            self.driver = webdriver.Chrome(self.path)
            self.pic_folder_path = 'C:\\Users\\Cass\\Desktop\\Python\\Data'
